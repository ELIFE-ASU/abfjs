...
const {SectionMap, ADC} = require('../src/header');
...
const build_table = function() {
    ...
    const json_files = abf_files.map((f) => path.basename(f, '.abf') + '-adc.json');
    ...
    return abf_files.map(function(abf_file, idx) {
        ...
        const got = ADC(data, SectionMap(data));
...
test.each(build_table())('.adc(%s)', function(filename, got, expected) {
