...
const {SectionMap, Protocol} = require('../src/header');
...
const build_table = function() {
    ...
    const json_files = abf_files.map((f) => path.basename(f, '.abf') + '-proto.json');
    ...
    return abf_files.map(function(abf_file, idx) {
        ...
        const got = Protocol(data, SectionMap(data));
...
test.each(build_table())('.protocol(%s)', function(filename, got, expected) {
