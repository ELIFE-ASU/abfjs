...
const build_table = function() {
    ...
    const json_files = abf_files.map((f) => path.basename(f, '.abf') + '-tag.json');
    ...
    return abf_files.map(function(abf_file, idx) {
        ...
        const got = Tag(data, SectionMap(data));
...
test.each(build_table())('.tag(%s)', function(filename, got, expected) {
