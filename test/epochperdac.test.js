...
const {SectionMap, EpochPerDAC} = require('../src/header');
...
const build_table = function() {
    ...
    const json_files = abf_files.map((f) => path.basename(f, '.abf') + '-epochperdac.json');
    ...
    return abf_files.map(function(abf_file, idx) {
        ...
        const got = EpochPerDAC(data, SectionMap(data));
...
test.each(build_table())('.epoch_per_dac(%s)', function(filename, got, expected) {
