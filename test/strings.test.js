...
const {SectionMap, Strings} = require('../src/header');
...
const build_table = function() {
    ...
    const json_files = abf_files.map((f) => path.basename(f, '.abf') + '-strings.json');
    ...
    return abf_files.map(function(abf_file, idx) {
        ...
        const got = Strings(data, SectionMap(data));
...
test.each(build_table())('.strings(%s)', function(filename, got, expected) {
